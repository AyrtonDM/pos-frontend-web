...
import { PagoService, Plan, CheckoutResponse } from '../../../../../../core/services/pago.service';
...
export class PlanSelectorModalComponent implements OnInit {
  ...
  cargarPlanes(): void {
    ...
    this.pagoService.getPlanes().subscribe({
      ...
      error: (err: any) => {
        this.errorPlanes = 'No se pudieron cargar los planes. Intente de nuevo más tarde.';
        this.cargandoPlanes = false;
  ...
  seleccionarPlan(plan: Plan): void {
    if (this.procesandoPagoId !== null) return;
