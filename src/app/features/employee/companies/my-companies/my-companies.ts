...
@Component({
  selector: 'app-employee-my-companies',
  imports: [Navbar, RouterLink],
  templateUrl: './my-companies.html',
  styleUrl: './my-companies.css',
})
export class EmployeeMyCompanies implements OnInit {
  ...
  protected obtenerIdEmpresa(company: Company): string | number {
    return company.idEmpresa ?? company.id_empresa ?? company.empresa_id ?? company.id ?? company.nit;
  }
  ...
  private cargarMisEmpresas(): void {
    ...
    this.companyService.getMisEmpresasEmpleado().subscribe({
      next: (companies) => {
